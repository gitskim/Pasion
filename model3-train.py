...
model = Sequential()
model.add(Bidirectional(LSTM(512, dropout=0.2), input_shape=(timeseries, input_dim)))
...
with open('model3-5-14-mse.json', 'w') as f:
    hist_df.to_json(f)
...
with open('model3-5-14-mae.json', 'w') as f:
    hist_df.to_json(f)
